...
function buildShadowUI() {
    ...
    [['tactical','TACTICAL OVERVIEW'],['reports','DOSSIER ARCHIVE'],['sources','SOURCE CONTROL'],['doctrine','SHADOW DOCTRINE']].forEach(([id,label], index) => {
    ...
    const collect=button('COLLECT NEXT SOURCES',()=>void collectShadow());collect.id='shadow-collect';const analyze=button('ANALYZE 40–60',()=>void analyzeShadow(false));analyze.id='shadow-analyze';const daily=button('GENERATE DAILY MASTER REPORT',()=>void analyzeShadow(true));daily.id='shadow-daily';actions.append(collect,analyze,daily);control.append(progressLabel,progress,actions);
    const stream=panel('shadow-intel-stream','LATEST INTERCEPTS','shadow-span-2');stream.append(el('div','shadow-list'));stream.lastChild.id='shadow-stream-list';
    ...
    tactical.append(mapPanel,control,regionList,conflictList,stream);
    ...
    shell.append(header,metrics,tabs,tactical,reports,sources,doctrine,flashNode);view.replaceChildren(shell);
}
...
export function initShadowOSINT() {
    ...
    window.setInterval(()=>{const view=document.getElementById('view-shadow');if(view&&!view.classList.contains('hidden'))void refreshShadowOSINT()},30000);
...
export async function refreshShadowOSINT() {
    ...
    try {
        ...
        shadowState={status,snapshot,reports:reports.reports||[],regions:regions.regions||[],conflictLinks:regions.conflict_links||[]};renderStatus();renderStream();renderRegions();renderConflictLinks();renderReports();renderSources();
        const prompt=document.getElementById('shadow-system-prompt');if(prompt&&document.activeElement!==prompt)prompt.value=snapshot.system_prompt||'';if(commandGlobe)commandGlobe.setData(shadowState.regions,shadowState.conflictLinks);flash('SHADOW command fabric synchronized.','success');
    } catch(error){flash(`SHADOW unavailable: ${error.message}`,'error')}
}

function renderStatus(){const s=shadowState.status;const values={sources:s.sources||0,enabled:s.enabled_sources||0,pending:s.pending_items||0,processed:s.processed_items||0,reports:s.reports||0,regions:shadowState.regions.length};Object.entries(values).forEach(([id,value])=>{const node=document.getElementById(`shadow-metric-${id}`);if(node)node.textContent=String(value)});const pct=Math.min(100,((s.pending_items||0)/(s.batch_min||40))*100);const bar=document.getElementById('shadow-progress-bar');if(bar)bar.style.width=`${pct}%`;const label=document.getElementById('shadow-progress-label');if(label)label.textContent=(s.pending_items||0)>=40?`${Math.min(s.pending_items,60)} ITEMS // ANALYSIS READY`:`${s.pending_items||0} / ${s.batch_min||40} ITEMS // ACCUMULATING`;const analyze=document.getElementById('shadow-analyze');if(analyze)analyze.disabled=(s.pending_items||0)<40||s.analysis_running;const daily=document.getElementById('shadow-daily');if(daily)daily.disabled=shadowState.reports.filter(report=>report.kind==='batch'&&new Date(report.created_at).toDateString()===new Date().toDateString()).length<2}
