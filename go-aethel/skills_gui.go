...
func (s *VisionContextSkill) RiskLevel() RiskLevel { return RiskModerate }
...
func (s *VisionContextSkill) Execute(args json.RawMessage) (string, error) {
	...
	case "screenshot":
		if err := CapturePrimaryDisplay(); err != nil {
			LogKernelActivity("VISION_SCREENSHOT_FAILED", "", "ERROR")
			return "", err
		}
		...
		LogKernelActivity("VISION_SCREENSHOT", fmt.Sprintf("%d bytes", len(data)), "SUCCESS")
...
func (s *GUIWindowControlSkill) RiskLevel() RiskLevel { return RiskCritical }
...
"@
		...
		case "focus", "move", "close":
			...
			if input.WindowID != "" {
				...
				// Search by title
				findScript := fmt.Sprintf(`Get-Process | Where-Object { $_.MainWindowTitle -like '*%s*' } | Select-Object -First 1 -ExpandProperty Id`, input.TitlePattern)
				cmdFind := exec.Command(getPowerShellPath(), "-NoProfile", "-NonInteractive", "-Command", findScript)
				cmdFind.SysProcAttr = &syscall.SysProcAttr{HideWindow: true}
				outFind, err := cmdFind.Output()
				if err == nil && len(strings.TrimSpace(string(outFind))) > 0 {
					targetID = strings.TrimSpace(string(outFind))
				}
			}
