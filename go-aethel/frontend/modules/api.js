...
export async function checkSystemStatus() {
    ...
    try {
        const res = await fetch(`${state.API_BASE}/health`, { signal: controller.signal });
        return await jsonResponse(res, 'Core health');
    } finally {
        clearTimeout(timer);
    }
}
...
export async function getModels() {
    // Prefer the virtual HTTP route because it includes live-discovered models.
    // The direct Wails binding survives startup races and asset-server faults.
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 4000);
    let fetchError = null;
    try {
        const res = await fetch(`${state.API_BASE}/v1/models`, { signal: controller.signal });
        const payload = await jsonResponse(res, 'Model registry');
        if (Array.isArray(payload?.models) && payload.models.length > 0) return payload;
    } catch (error) {
        fetchError = error;
    } finally {
        clearTimeout(timer);
    ...
    const binding = window.go?.main?.App?.GetAvailableModels;
    if (typeof binding === 'function') {
        const models = await binding();
...
export async function getPersonalStatus() {
    ...
    try {
        const res = await fetch(`${state.API_BASE}/v1/personal/status`, { signal: controller.signal });
        if (!res.ok) throw new Error(`Personal status (${res.status})`);
