...
func (s *ReleaseService) Preferences() (ReleasePreferences, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	data, _, err := readSealedFile(s.path)
...
func (s *ReleaseService) SavePreferences(preferences ReleasePreferences) error {
	...
	s.mu.Lock()
	defer s.mu.Unlock()
	return writeSealedFile(s.path, data)
}
...
func appendSealedJSONLine(path string, item []byte) error {
	var lines [][]byte
	data, _, err := readSealedFile(path)
	...
	return writeSealedFile(path, encoded)
}
...
func handleReleaseInfo(w http.ResponseWriter, r *http.Request) {
	...
	json.NewEncoder(w).Encode(map[string]interface{}{"product": ProductName, "version": ProductVersion, "update_mode": "signed_manifest_manual_install"})
}
