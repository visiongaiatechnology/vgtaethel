...
func dpapiProtect(plaintext []byte) ([]byte, error) {
	...
	if uint64(len(plaintext)) > uint64(^uint32(0)) {
		return nil, errors.New("DPAPI plaintext exceeds platform boundary")
	}
	in := windows.DataBlob{Size: uint32(len(plaintext)), Data: &plaintext[0]} // #nosec G115 -- explicit uint32 bound above.
...
func dpapiUnprotect(encoded []byte) ([]byte, error) {
	...
	if uint64(len(ciphertext)) > uint64(^uint32(0)) {
		return nil, errors.New("DPAPI payload exceeds platform boundary")
	}
	in := windows.DataBlob{Size: uint32(len(ciphertext)), Data: &ciphertext[0]} // #nosec G115 -- explicit uint32 bound above.
	var out windows.DataBlob
