...
func handleRunsPath(w http.ResponseWriter, r *http.Request) {
	...
	if len(parts) == 0 || parts[0] == "" || len(parts) > 2 || !safeResourceIDPattern.MatchString(parts[0]) {
...
func handleArtifacts(w http.ResponseWriter, r *http.Request) {
	...
	artifacts, err := defaultFileSnapshots.ListArtifacts()
	if err != nil {
