...
func (s *CodeCartographySkill) RiskLevel() security.RiskLevel { return security.RiskModerate }
...
func (s *CodeCartographySkill) Execute(args json.RawMessage) (string, error) {
	...
	safeOutput, err := security.ValidateWritePath(output)
	if err != nil {
		return "", err
	}
	...
	snapshotID, err := system.DefaultFileSnapshots.Create(safeOutput)
	if err != nil {
		return "", err
	}
	...
	state.RecordFileChange(safeOutput, len(strings.Split(report, "\n")), 0)
	security.LogKernelActivity("CODE_CARTOGRAPHY", root, "SUCCESS")
...
func collectCartographyFiles(root string, maxFiles int) ([]cartographyFile, int, error) {
	...
	err = fs.WalkDir(rootHandle.FS(), ".", func(path string, entry fs.DirEntry, walkErr error) error {
		...
		if entry.IsDir() {
			if path != "." && isCartographyIgnoredDir(entry.Name()) {
				return fs.SkipDir
			}
			return nil
		}
		...
		data, err := rootHandle.ReadFile(filepath.ToSlash(path))
		if err != nil || !isLikelyText(data) {
			skipped++
			return nil
		}
		files = append(files, analyzeCartographyFile(filepath.ToSlash(path), language, data, info.Size()))
		return nil
	})
