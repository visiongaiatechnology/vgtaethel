...
async function startTeamRun() {
    ...
    const prompt = [
        document.getElementById('agent-prompt-orchestrator')?.value,
        document.getElementById('agent-prompt-builder')?.value,
        'Arbeite als persistenter Agenten-Run: plane, führe nur zulässige Tools aus, verifiziere jeden Schritt und liefere einen Abschlussbericht.'
    ].filter(Boolean).join('\n\n');
    try {
        const response = await fetch(`${state.API_BASE}/v1/chat/runs`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ objective, profile_id: 'developer', model_id: modelID, system_prompt: prompt, messages: [{ role: 'user', content: objective }], cost_budget_usd: 5, max_agent_turns: 24 }) });
