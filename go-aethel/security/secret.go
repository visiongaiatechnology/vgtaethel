...
func NewSecretVault(filePath, keyPath string) (*SecretVault, error) {
	resolvedFile, err := CanonicalTarget(filePath)
	if err != nil {
		return nil, err
	}
	resolvedKey, err := CanonicalTarget(keyPath)
	if err != nil {
		return nil, err
	}
	vault := &SecretVault{
...
func (sv *SecretVault) initKey() error {
	if err := os.MkdirAll(filepath.Dir(sv.keyPath), 0700); err != nil {
		return err
	}
	protectedPath := sv.keyPath + ".protected"
	if protected, err := os.ReadFile(protectedPath); err == nil {
		key, unwrapErr := unprotectVaultDataKey(protected)
		if unwrapErr != nil || len(key) != 32 {
			return fmt.Errorf("protected vault key cannot be opened")
		}
		sv.key = key
		return nil
	} else if !os.IsNotExist(err) {
		return fmt.Errorf("failed to read protected vault key: %w", err)
	}

	// One-time migration from the historical plaintext vault.key. It is removed
	// only after the DPAPI/platform-protected envelope was durably written.
	key, err := os.ReadFile(sv.keyPath)
	if err == nil {
		if len(key) != 32 {
			return fmt.Errorf("invalid vault key length")
		}
		protected, protectErr := protectVaultDataKey(key)
		if protectErr != nil {
			return protectErr
		}
		if writeErr := os.WriteFile(protectedPath, protected, 0600); writeErr != nil { // #nosec G703 -- constructor canonicalizes this application-owned private path.
			return writeErr
		}
		if removeErr := os.Remove(sv.keyPath); removeErr != nil {
			return fmt.Errorf("protected vault key created but legacy plaintext key removal failed: %w", removeErr)
		}
		sv.key = append([]byte(nil), key...)
		return nil
	}
	if !os.IsNotExist(err) {
	...
	protected, protectErr := protectVaultDataKey(newKey)
	if protectErr != nil {
		return fmt.Errorf("failed to protect new vault key: %w", protectErr)
	}
	err = os.WriteFile(protectedPath, protected, 0600)
	if err != nil {
		return fmt.Errorf("failed to save protected key: %v", err)
	}
