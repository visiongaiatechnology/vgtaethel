...
func handleSecurityLeases(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodGet {
		...
		allowedCapabilities := map[security.Capability]bool{
			security.CapSysExec: true, security.CapFsRead: true, security.CapFsWrite: true, security.CapFsMount: true,
			security.CapGuiMoveMouse: true, security.CapGuiClick: true, security.CapGuiType: true, security.CapGuiPressKey: true,
			security.CapMediaControl: true, security.CapBrowserOpen: true, security.CapBrowserCtl: true, security.CapScreenRead: true,
			security.CapMemoryRead: true, security.CapMemoryWrite: true, security.CapCodexHandoff: true,
		}
		capability := security.Capability(req.Capability)
		...
		if lease.Capability == security.CapSysExec {
			lease.Scope.ForbiddenTargets = []string{"rm", "dd", "mkfs", "format", "shred", "wipe"}
		} else if lease.Capability == security.CapGuiPressKey {
