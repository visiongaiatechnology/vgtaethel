...
func TestUSGSEarthquakeAdapterPreservesGeospatialCoordinates(t *testing.T) {
	...
	feed := `{"features":[
		{"id":"abc","properties":{"title":"M 5.1 - Test","url":"https://example.invalid/event","mag":5.1,"time":1700000000000},"geometry":{"coordinates":[7.2,50.7,10]}},
		{"id":"bad","properties":{"title":"M 3.0","url":"https://example.invalid/bad","mag":3.0,"time":1700000000000},"geometry":{"coordinates":[999,999]}}
	]}`
	...
	if !strings.Contains(events[0].Title, "[earthquake]") {
		t.Fatalf("title must carry earthquake tag: %q", events[0].Title)
	}
}
