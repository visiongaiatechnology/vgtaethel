...
func (failingReadScopeSkill) RiskLevel() RiskLevel               { return RiskLow }
...
func TestRunEnginePersistsControlledLifecycle(t *testing.T) {
	...
	if run, err = engine.Advance(run.ID, NewPolicyEngine(NewSecurityGuard(), NewLeaseManager(filepath.Join(t.TempDir(), "leases.json")), NewAuditLogger(filepath.Join(t.TempDir(), "audit.json"))), NewSkillRegistry()); err != nil {
	...
	if _, err := engine.Advance(run.ID, NewPolicyEngine(NewSecurityGuard(), NewLeaseManager(filepath.Join(t.TempDir(), "leases2.json")), NewAuditLogger(filepath.Join(t.TempDir(), "audit2.json"))), NewSkillRegistry()); err != nil {
		t.Fatalf("report advance failed: %v", err)
	}
	if run, err = engine.Advance(run.ID, NewPolicyEngine(NewSecurityGuard(), NewLeaseManager(filepath.Join(t.TempDir(), "leases3.json")), NewAuditLogger(filepath.Join(t.TempDir(), "audit3.json"))), NewSkillRegistry()); err != nil {
...
func TestReadScopeJailErrorReturnsRecoveryContextInsteadOfFailingRun(t *testing.T) {
	...
	policy := NewPolicyEngine(NewSecurityGuard(), NewLeaseManager(filepath.Join(t.TempDir(), "leases.json")), NewAuditLogger(filepath.Join(t.TempDir(), "audit.json")))
...
func TestRunEngineWaitsForApprovalBeforeToolExecution(t *testing.T) {
	engine := NewRunEngine(filepath.Join(t.TempDir(), "runs.json"))
	args, err := json.Marshal(BrowserArgs{Action: "search", SearchQuery: "Aethel Produktstatus"})
	if err != nil {
		t.Fatal(err)
	}
	run, err := engine.Create(CreateRunRequest{
	...
	run, err = engine.Advance(run.ID, NewPolicyEngine(NewSecurityGuard(), NewLeaseManager(filepath.Join(t.TempDir(), "leases.json")), NewAuditLogger(filepath.Join(t.TempDir(), "audit.json"))), registry)
	if err != nil {
...
func TestRunApprovalIsBoundToPendingRunStep(t *testing.T) {
	...
	args, _ := json.Marshal(BrowserArgs{Action: "search", SearchQuery: "Aethel"})
	...
	policy := NewPolicyEngine(NewSecurityGuard(), NewLeaseManager(filepath.Join(temp, "leases.json")), NewAuditLogger(filepath.Join(temp, "audit.json")))
	registry := NewSkillRegistry()
	registry.Register(&WebBrowserSkill{})
	run, err = engine.Advance(run.ID, policy, registry)
	...
	approvals := NewApprovalManager(filepath.Join(temp, "approvals.json"))
