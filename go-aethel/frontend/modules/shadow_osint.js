...
function buildShadowUI() {
    ...
    const collect=button('COLLECT NEXT SOURCES',()=>void collectShadow());collect.id='shadow-collect';const analyze=button('ANALYZE 40–60',()=>void analyzeShadow(false));analyze.id='shadow-analyze';const autonomy=button('AUTONOMY: OFF',()=>void toggleShadowAutonomy());autonomy.id='shadow-autonomy';autonomy.setAttribute('aria-pressed','false');const daily=button('GENERATE DAILY MASTER REPORT',()=>void analyzeShadow(true));daily.id='shadow-daily';const analysisState=el('div','shadow-analysis-state','MANUAL CONTROL READY');analysisState.id='shadow-analysis-state';analysisState.setAttribute('role','status');analysisState.setAttribute('aria-live','polite');actions.append(collect,analyze,autonomy,daily);control.append(progressLabel,progress,actions,analysisState);
...
function renderStatus(){const s=shadowState.status;const values={sources:s.sources||0,enabled:s.enabled_sources||0,pending:s.pending_items||0,processed:s.processed_items||0,reports:s.reports||0,regions:shadowState.regions.length};Object.entries(values).forEach(([id,value])=>{const node=document.getElementById(`shadow-metric-${id}`);if(node)node.textContent=String(value)});const pct=Math.min(100,((s.pending_items||0)/(s.batch_min||40))*100);const bar=document.getElementById('shadow-progress-bar');if(bar)bar.style.width=`${pct}%`;const label=document.getElementById('shadow-progress-label');if(label)label.textContent=s.analysis_running?`AI ANALYSIS RUNNING // ${s.analysis_model_id||'SELECTED CORE'}`:(s.pending_items||0)>=40?`${Math.min(s.pending_items,60)} ITEMS // ANALYSIS READY`:`${s.pending_items||0} / ${s.batch_min||40} ITEMS // ACCUMULATING`;const analyze=document.getElementById('shadow-analyze');if(analyze){analyze.disabled=(s.pending_items||0)<40||s.analysis_running;analyze.title=s.analysis_running?'An analysis is already running.':(s.pending_items||0)<40?`At least ${s.batch_min||40} pending items required.`:'Start one evidence-bound batch.'}const autonomy=document.getElementById('shadow-autonomy');if(autonomy){autonomy.textContent=`AUTONOMY: ${s.autonomy_enabled?'ON':'OFF'}`;autonomy.classList.toggle('active',Boolean(s.autonomy_enabled));autonomy.setAttribute('aria-pressed',String(Boolean(s.autonomy_enabled)))}const state=document.getElementById('shadow-analysis-state');if(state){state.dataset.level=s.last_analysis_error?'error':s.analysis_running?'running':'ready';state.textContent=s.last_analysis_error?`LAST RUN FAILED // ${s.last_analysis_error}`:s.analysis_running?`${s.autonomy_enabled?'AUTONOMOUS':'MANUAL'} DOSSIER GENERATION IN PROGRESS${s.analysis_started_at?` // SINCE ${new Date(s.analysis_started_at).toLocaleTimeString()}`:''}`:s.last_analysis_at?`LAST ANALYSIS ${new Date(s.last_analysis_at).toLocaleString()} // READY`:`${s.autonomy_enabled?'AUTONOMOUS COLLECTION + BATCH ANALYSIS ARMED':'MANUAL CONTROL READY'}`}const daily=document.getElementById('shadow-daily');if(daily)daily.disabled=s.analysis_running||shadowState.reports.filter(report=>report.kind==='batch'&&new Date(report.created_at).toDateString()===new Date().toDateString()).length<2}
