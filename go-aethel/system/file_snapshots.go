...
func (s *FileSnapshotStore) Create(path string) (string, error) {
	safePath, err := security.CanonicalTarget(path)
	if err != nil {
		return "", err
	}
	data, err := os.ReadFile(safePath) // #nosec G304 -- canonical application path; authorization is enforced by the calling write skill.
	existed := err == nil
	...
	payload, err := json.Marshal(FileSnapshot{ID: id, Path: safePath, Existed: existed, Content: data, CreatedAt: time.Now().UTC()})
	if err != nil {
		return "", err
	}
	s.mu.Lock()
	...
	return id, security.WriteSealedFile(filepath.Join(s.dir, id+".sealed"), payload)
}
func (s *FileSnapshotStore) Restore(id string) error {
	...
	s.mu.Lock()
	defer s.mu.Unlock()
	data, _, err := security.ReadSealedFile(filepath.Join(s.dir, id+".sealed"))
	if err != nil {
		return err
	}
	...
	root, name, safe, err := security.OpenAuthorizedFile(snap.Path, security.MountWrite)
	...
	return root.WriteFile(name, snap.Content, 0600)
}
...
func (s *FileSnapshotStore) ListArtifacts() ([]FileSnapshotArtifact, error) {
	...
	for _, entry := range entries {
		...
		data, _, readErr := security.ReadSealedFile(filepath.Join(s.dir, entry.Name()))
		if readErr != nil {
			continue
		}
...
func (s *RestoreSnapshotSkill) RiskLevel() security.RiskLevel { return security.RiskCritical }
