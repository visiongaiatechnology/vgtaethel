...
func (s *ReadFileSkill) RiskLevel() security.RiskLevel { return security.RiskSafe }
...
func (s *ReadFileSkill) Execute(args json.RawMessage) (string, error) {
	...
	if err != nil {
		security.LogKernelActivity("READ_FAILED", input.Path, "ERROR")
		return "", err
	}
	security.LogKernelActivity("READ", input.Path, "SUCCESS")
...
func (s *WriteFileSkill) RiskLevel() security.RiskLevel { return security.RiskCritical }
...
func (s *WriteFileSkill) Execute(args json.RawMessage) (string, error) {
	...
	safePath, err := security.ValidateWritePath(input.Path)
	if err != nil {
		return "", err
	}
	snapshotID, err := system.DefaultFileSnapshots.Create(safePath)
	if err != nil {
		return "", err
	}
	...
	if err := os.WriteFile(safePath, []byte(input.Content), 0600); err != nil {
		security.LogKernelActivity("WRITE_FAILED", input.Path, "ERROR")
		return "", err
	}
	security.LogKernelActivity("WRITE", input.Path, "SUCCESS")

	// Compute and record line changes
	added, removed := computeLineChanges(oldContent, input.Content)
	state.RecordFileChange(safePath, added, removed)
...
func (s *ReplaceFileContentSkill) RiskLevel() security.RiskLevel { return security.RiskCritical }
...
func (s *ReplaceFileContentSkill) Execute(args json.RawMessage) (string, error) {
	...
	fileRoot, fileName, safePath, err := security.OpenAuthorizedFile(input.Path, security.MountWrite)
	if err != nil {
		return "", err
	}
	defer fileRoot.Close()

	data, err := fileRoot.ReadFile(fileName)
	if err != nil {
		return "", err
	}
	oldContent := string(data)
	...
	if err := fileRoot.WriteFile(fileName, []byte(newContent), 0600); err != nil {
		security.LogKernelActivity("WRITE_FAILED", input.Path, "ERROR")
		return "", err
	}
	security.LogKernelActivity("WRITE", input.Path, "SUCCESS")
...
func (s *ListDirSkill) RiskLevel() security.RiskLevel { return security.RiskSafe }
...
func (s *ListDirSkill) Execute(args json.RawMessage) (string, error) {
	...
	if err != nil {
		security.LogKernelActivity("LIST_DIR_FAILED", input.Path, "ERROR")
		return "", err
	}
	...
	security.LogKernelActivity("LIST_DIR", input.Path, "SUCCESS")
	if len(fileInfos) == 0 {
...
func (s *MountFolderSkill) RiskLevel() security.RiskLevel { return security.RiskCritical }

func (s *MountFolderSkill) Parameters() map[string]interface{} {
	return map[string]interface{}{
		"type": "object",
		"properties": map[string]interface{}{
			"access":           map[string]interface{}{"type": "string", "enum": []string{"read"}},
			"duration_minutes": map[string]interface{}{"type": "integer", "minimum": 1, "maximum": 60},
...
func (s *MountFolderSkill) Execute(args json.RawMessage) (string, error) {
	...
	if err != nil {
		security.LogKernelActivity("MOUNT_FAILED", input.Path, "ERROR")
		return "", fmt.Errorf("Fehler beim Speichern der Einhängung: %v", err)
	}

	security.LogKernelActivity("MOUNT_FOLDER", input.Path, "SUCCESS")
