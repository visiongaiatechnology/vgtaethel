...
/**
 * Fallback für Markdown-Tabellen und Grundformatierung, falls marked nicht lädt
 */
...
function updateContextUtilization(inputTokens, outputTokens, cachedTokens = 0, modelID = state.currentModel) {
    const model = state.modelRegistry.find(candidate => candidate.id === modelID);
    const limit = Number(model?.context_tokens || 0);
    const used = Math.max(0, Number(inputTokens || 0)) + Math.max(0, Number(outputTokens || 0));
    const element = document.getElementById('context-utilization');
    if (!element) return;
    const limitLabel = limit >= 1000000
        ? `${(limit / 1000000).toFixed(limit % 1000000 === 0 ? 0 : 2)}M`
        : (limit > 0 ? `${Math.round(limit / 1024)}k` : '—');
    const utilization = limit > 0 ? ` (${Math.min(100, (used / limit) * 100).toFixed(2)}%)` : '';
    const cache = Number(cachedTokens || 0) > 0 ? ` [⚡ Cache: ${Number(cachedTokens).toLocaleString()}]` : '';
    element.textContent = `${used.toLocaleString()} / ${limitLabel}${utilization}${cache}`;
}
...
async function sendMessageViaPersistentRun() {
    ...
    const globalWatchRequest = prompt.startsWith('[GLOBAL WATCH OPERATOR]');
    const profile = state.isSphereActive ? 'sphere_workspace' : (controlActive ? 'browser_operator' : (globalWatchRequest ? 'global_watch_operator' : 'aethel_runtime'));
    ...
    const createRes = await fetch(`${state.API_BASE}/v1/chat/runs`, {
        ...
        body: JSON.stringify({
            ...
            orchestrator_model_id: state.orchestratorModel || state.currentModel,
    ...
    for (let poll = 0; poll < 1200; poll++) {
        ...
        const tokenBudget = run?.continuity?.token_budget;
        if (tokenBudget) {
            updateContextUtilization(tokenBudget.input_tokens, tokenBudget.output_tokens, tokenBudget.cached_tokens, run.model_id || state.currentModel);
        }
        ...
        if (run.status === 'waiting_approval') {
            ...
            if ((run.steps || []).some(step => step.tool_name === 'sphere_write_document' && step.status === 'verified')) {
                window.dispatchEvent(new CustomEvent('aethel:sphere-document-updated'));
            }
                ...
                if (line.startsWith("data:")) {
                    ...
                    if (data.startsWith("[[TOOL_DELTA]]:")) {
                        ...
                        try {
                            ...
                            const activeModel = state.modelRegistry.find(candidate => candidate.id === state.currentModel);
                            const maxContext = Math.max(1, Number(activeModel?.context_tokens || 0));
...
export async function loadSessionsList() {
    ...
    loading.textContent = 'Chat-Archiv wird synchronisiert…';
    ...
    try {
        ...
        if (!sessions || sessions.length === 0) {
            ...
            empty.textContent = 'Keine archivierten Chat-Impulse vorhanden.';
            container.appendChild(empty);
            return;
        }
        for (const session of sessions) {
            const card = document.createElement('article');
            card.className = `glass-card archive-card${session.id === state.currentSessionId ? ' active' : ''}`;
            const heading = document.createElement('div');
            heading.className = 'archive-card-heading';
            const title = document.createElement('strong');
            title.textContent = session.title || 'Unbenannter Chat';
            ...
            remove.addEventListener('click', async () => {
                if (remove.dataset.armed !== 'true') {
                    ...
                    remove.textContent = 'BESTÄTIGEN';
                    setTimeout(() => {
                        if (remove.isConnected && remove.dataset.armed === 'true') {
            ...
            date.textContent = session.timestamp || 'Unbekannt';
            const id = document.createElement('code');
            id.textContent = session.id || '—';
            meta.append(date, id);
            const load = document.createElement('button');
            load.type = 'button';
            load.className = 'cyber-button archive-load';
            load.textContent = session.id === state.currentSessionId ? 'AKTIVER CHAT' : 'CHAT LADEN';
            load.disabled = session.id === state.currentSessionId;
            load.addEventListener('click', () => void loadSessionAndSwitch(session.id));
            card.append(heading, meta, load);
        ...
        failure.className = 'archive-empty archive-error';
        failure.textContent = `Chat-Archiv nicht erreichbar: ${e.message}`;
        container.appendChild(failure);
    }
}

async function loadSessionAndSwitch(id) {
    await loadSession(id);
    import('./ui.js').then(m => m.switchMode("chat"));
}
window.loadSessionAndSwitch = loadSessionAndSwitch;
...
export async function deleteSession(id) {
    try {
        const response = await fetch(`${state.API_BASE}/v1/chat/sessions/delete?id=${encodeURIComponent(id)}`, {
            method: 'DELETE'
        });
        ...
        window.showAethelToast?.('Chat wurde aus dem lokalen Archiv gelöscht.', 'success');
    } catch(e) {
        console.error("Failed to delete session", e);
        window.showAethelToast?.(`Chat konnte nicht gelöscht werden: ${e.message}`, 'error');
