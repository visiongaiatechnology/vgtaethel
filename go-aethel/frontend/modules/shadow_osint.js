...
function buildShadowUI() {
    ...
    const headerStatus=el('div','shadow-header-status');const live=el('span','shadow-live','LIVE INTELLIGENCE');const classification=el('div','shadow-classification','BETA V4 // LOCAL SOVEREIGN');headerStatus.append(live,classification);header.append(identity,headerStatus);
    ...
    const mapWrap = el('div', 'shadow-map-wrap'); const globeHost = el('div', 'shadow-map'); globeHost.id = 'shadow-conflict-globe';
    const legend = el('div', 'shadow-map-legend'); [['#38c878','STABLE'],['#e6c84c','TENSION'],['#ff7a2f','ESCALATION'],['#ff244d','WAR'],['#b84dff','CYBER']].forEach(([color,label]) => { const item=el('span'); const dot=el('i'); dot.style.backgroundColor=color; item.append(dot,document.createTextNode(label)); legend.append(item); }); mapPanel.append(mapWrap);
    const telemetry=el('div','shadow-globe-telemetry');telemetry.append(el('span','','CESIUM // 3D GEOINT'),el('span','','DRAG TO ROTATE'),el('span','','WHEEL TO ZOOM'));const selection=el('aside','shadow-globe-selection','SELECT A REGION OR CONFLICT VECTOR');selection.id='shadow-globe-selection';mapWrap.replaceChildren(globeHost,telemetry,selection,legend);
...
export function initShadowOSINT() {
    ...
    const refreshWhenVisible=()=>{const view=document.getElementById('view-shadow');if(view&&!view.classList.contains('hidden'))void refreshShadowOSINT();window.setTimeout(refreshWhenVisible,document.hidden?30000:5000)};window.setTimeout(refreshWhenVisible,5000);
    window.addEventListener('aethel:shadow-open-report',event=>openShadowReport(String(event.detail?.reportID||'')));
...
export async function refreshShadowOSINT() {
    ...
    try {
        ...
        shadowState={status,snapshot,reports:reports.reports||[],regions:regions.regions||[],conflictLinks:regions.conflict_links||[]};renderStatus();renderStream();renderRegions();renderConflictLinks();renderReports();renderForecastMatrix();renderLatestDossiers();renderSources();openShadowReport(sessionStorage.getItem('aethel.shadow.openReport')||'');
...
function openShadowReport(reportID){if(!reportID)return;const report=shadowState.reports.find(item=>item.id===reportID);if(!report)return;activateTab('reports');const reader=document.getElementById('shadow-report-body');if(reader)reader.textContent=reportText(report);sessionStorage.removeItem('aethel.shadow.openReport')}
