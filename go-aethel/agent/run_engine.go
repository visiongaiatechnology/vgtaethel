...
type AgentProfile struct {
	...
	AllowedCapabilities []security.Capability `json:"allowed_capabilities"`
...
func (e *RunEngine) load() error {
	e.mu.Lock()
	defer e.mu.Unlock()
	data, _, err := security.ReadSealedFile(e.filePath)
	...
	for _, run := range list {
		...
		if run.Continuity.Tier == "" || run.Continuity.TokenBudget.MaxTotalTokens == 0 {
			migrated := newContinuityState(run.Objective, run.SphereActive)
			migrated.TokenBudget.InputTokens = run.Continuity.TokenBudget.InputTokens
...
func (e *RunEngine) saveLocked() error {
	...
	return security.WriteSealedFile(e.filePath, data)
}
...
func (e *RunEngine) Profiles() []AgentProfile {
	...
	for _, profile := range e.profiles {
		profile.AllowedCapabilities = append([]security.Capability(nil), profile.AllowedCapabilities...)
...
func (e *RunEngine) Create(input CreateRunRequest) (AgentRun, error) {
	...
	if len(run.Continuity.ExpectedEffects) > 0 && run.Continuity.Tier == CognitiveDirect {
		...
		run.Continuity.TokenBudget = defaultTokenBudget(CognitiveOperational)
	}
...
func (e *RunEngine) RecordCost(id string, costUSD float64) (AgentRun, error) {
	run, err := e.RecordInference(id, 0, 0, 0, costUSD)
	if err != nil || run.SpentUSD <= run.CostBudgetUSD {
		return run, err
	}
	e.mu.Lock()
	defer e.mu.Unlock()
	run = e.runs[id]
	run.Status = RunPaused
	run.Continuity.Phase = PhasePaused
...
func (e *RunEngine) RecordInference(id string, inputTokens, outputTokens, cachedTokens int, costUSD float64) (AgentRun, error) {
	if costUSD < 0 {
		return AgentRun{}, errors.New("negative run cost is invalid")
	}
	if inputTokens < 0 || outputTokens < 0 || cachedTokens < 0 {
		return AgentRun{}, errors.New("negative token usage is invalid")
	}
	e.mu.Lock()
	defer e.mu.Unlock()
	run, ok := e.runs[id]
	...
	tokenLimitReached := run.Continuity.recordUsage(inputTokens, outputTokens, cachedTokens)
	if run.SpentUSD > run.CostBudgetUSD || tokenLimitReached {
		run.Trace = appendRunTrace(run.Trace, "budget_exhausted", "The current response was retained; another inference requires operator review.", "")
	}
	run.UpdatedAt = time.Now().UTC()
	e.runs[id] = run
	if err := e.saveLocked(); err != nil {
...
// PauseForExhaustedBudget is called immediately before provider inference. A
// completed provider response is never discarded merely because it crossed a
// budget boundary.
func (e *RunEngine) PauseForExhaustedBudget(id string) (AgentRun, bool, error) {
	e.mu.Lock()
	defer e.mu.Unlock()
	run, ok := e.runs[id]
	if !ok {
		return AgentRun{}, false, errors.New("run not found")
	}
	exhausted := run.Continuity.TokenBudget.Exhausted || run.SpentUSD > run.CostBudgetUSD
	if !exhausted || run.Status != RunRunning {
		return cloneRun(run), false, nil
	}
	run.Status = RunPaused
	run.Continuity.Phase = PhasePaused
	run.UpdatedAt = time.Now().UTC()
	run.Trace = appendRunTrace(run.Trace, "budget_paused", "Run paused before an additional provider inference.", "")
	e.runs[id] = run
	if err := e.saveLocked(); err != nil {
...
func (e *RunEngine) AppendAgentToolSteps(id string, calls []AgentToolCall) (AgentRun, error) {
	...
	for _, call := range calls {
		...
		if err := run.Continuity.registerAction(call.Name, call.Arguments); err != nil {
			run.Status = RunPaused
			run.Continuity.Phase = PhasePaused
			run.Trace = appendRunTrace(run.Trace, "loop_guard_paused", err.Error(), "")
			e.runs[id] = run
			if saveErr := e.saveLocked(); saveErr != nil {
				return AgentRun{}, saveErr
			}
	...
	run.Continuity.Phase = PhaseExecuting
	run.UpdatedAt = time.Now().UTC()
	e.runs[id] = run
	if err := e.saveLocked(); err != nil {
		return AgentRun{}, err
...
func (e *RunEngine) Approve(id, token string, approvals *security.ApprovalManager, policy *security.PolicyEngine) (AgentRun, error) {
	e.mu.Lock()
	defer e.mu.Unlock()
	run, ok := e.runs[id]
	if !ok {
...
func (e *RunEngine) Start(id string) (AgentRun, error) {
	_, err := e.transition(id, map[RunStatus]bool{RunQueued: true, RunPaused: true}, RunRunning, "started", "Operator started or resumed run.")
	if err != nil {
		return AgentRun{}, err
	}
	e.mu.Lock()
	current := e.runs[id]
	current.Continuity.Phase = PhasePlanning
...
func (e *RunEngine) Advance(id string, policy *security.PolicyEngine, registry *skills.SkillRegistry) (AgentRun, error) {
	e.mu.Lock()
	run, ok := e.runs[id]
	if !ok {
		e.mu.Unlock()
...
func executeRunStep(step RunStep, profile AgentProfile, policy *security.PolicyEngine, registry *skills.SkillRegistry) (result string, waitingApproval bool, err error) {
	switch step.Kind {
	case RunStepPlan:
	...
	case RunStepTool:
		...
		if toolErr == nil && step.ExpectedContains != "" && !strings.Contains(toolResult, step.ExpectedContains) {
			toolErr = fmt.Errorf("verification failed: expected result marker was not observed")
		}
		// U7: AgentAction trail from RunEngine (same as tools/execute HTTP path)
		if intelligence.SharedIntelStore != nil {
			status := "completed"
			resText := toolResult
			if toolErr != nil {
				status = "failed"
				resText = toolErr.Error()
			}
			approvedBy := ""
			if step.ApprovalGranted {
				approvedBy = "operator"
			}
			intelligence.SharedIntelStore.RecordAgentAction(step.ToolName, string(step.ToolArgs), resText, approvedBy, status)
		}
...
func profileAllows(profile AgentProfile, capability security.Capability) bool {
	// A zero capability means the scanner has no specific mapping for this tool.
	// The security.PolicyEngine already gates such tools via the security.RiskLevel path; there is
...
func desktopEvidenceForStep(step RunStep) string {
	...
	if err := skills.CapturePrimaryDisplay(); err != nil {
		return "unavailable"
	}
